...
public class D_Treasure {
    ...
    static void findSolution() {
        ...
        if (!acidTest2()) { // test if after opening all matching chests with the keys we have the obtained keys will open the rest
            return;
        }
    ...
    static boolean acidTest() {
        ...
        // check if existing keys won't open any of the remaining chests
        boolean ranOutOfKeys = true;
...
//            } else {
            if (cl != 0) {  // the chest is still full (locked)
                if (keys.contains(cl)) {
                    ranOutOfKeys = false;
                }    // still have some keys matching the chests
            ...
            if (cl != 0) {
                ...
                if (indx == -1) {
                    ...
                    if (chests.get(i).contains(cl)) {
                        ...
                        if (!tc.contains(cl)) {
                            return false;
                        } // it was the only key so the chest is unreachable                        
                    }
                }
            }
        }

        // check if sone other keys are unreachable


        return true;
    }

    static boolean acidTest2() {
        // 1. open all chests we have matching keys for and retrieve all keys in them
        // then check if any of the locks don't match the keys on hands - means impossible to solve


        // test if after opening all matching chests with the keys we have the obtained keys will open the rest
        ArrayList<Integer> tc = new ArrayList<Integer>(200);
        ArrayList<Integer> tk = new ArrayList<Integer>(400);
        tk.addAll(keys);
        ...
        for (int i = 0; i < n; i++) {
            int cl = chestLocks[i];
            if (cl != 0) {
                if (keys.contains(cl)) { // can open - get the keys
                    tk.addAll(chests.get(i));
                } else {
                    tc.add(cl);
                }
        ...
        for (int i = 0; i < tc.size(); i++) {
            if (!(tk.contains(tc.get(i)))) // have no key to open a chest
            {
                return false;
            }
